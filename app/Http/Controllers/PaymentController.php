...
class PaymentController extends Controller
{
   ...
   public function  index(Request $request) {
         ...
         if ($request->ajax()) {     
                 ...
                 $data =  CompanyPaid::all();
          
            return Datatables::of($data)
                ->addColumn('action', function ($user) {
            
                   $edit= route('payments.edit',['id'=> $user->id ]);
         ...
         return view('payments.index', compact('cdt'));
   }  
       ...
       // $users = DB::table('designer')->pluck( 'name' , 'id'); removed on 06-02-21 for designer table removing
        ...
        $vendors = Vendor::pluck('name', 'name');
          ...
          $perms = DB::table('permissions')
           ...
           $companynames = DB::table('clients')->pluck( 'client_company' , 'company_id');    


           $invoices =array();
        
       
           return view('payments.create', compact('users',  'status','perms','rights','vendors' , 'us_time', 'cdt' , 'target_date' , 'blank_array' ,'user_names', 'companynames','invoices'));
...
public function edit($id)
{
            ...
            $companypaid=CompanyPaid::find($id);

            $companypaid->status= 'Cancel';

            $amtpaid =  $companypaid->amt_paid_usd ;

            $invoice_no = $companypaid->invoices;

            $company_id =  $companypaid->company_id;

            $companypaid->save();

            $invsummary_paid = InvoiceSummary::where('company_id',$company_id)->where('invoice_no', $invoice_no)->select('amt_paid_usd')->get();

            //dd($invsummary_paid);

            $invsum_paid = 0;
            foreach ($invsummary_paid as $key ) {
               $invsum_paid =  $key->amt_paid_usd;
            ...
            InvoiceSummary::where('company_id',$company_id)->where('invoice_no', $invoice_no)->update(['amt_paid_usd' => $invsum_paid - $amtpaid]);


        return redirect()->back();         
              //    return view('payments.edit',compact('companypaid'));
...
public function searchinvoice(Request $request)
{
  ...
  //  $invoices = InvoiceSummary::where('company_id', $request->pcompany)->pluck('invoice_no', 'invoice_no');

     $invoices = InvoiceSummary::where('company_id', $request->pcompany)->get();


    //$html='<select>';

   //$html = '<option  value=0 >' . "Inv-no" .'|'. "Inv-Amount"  .   '</option> ';


             // $output = '<tr><td>1</td><td>2</td><td>3</td><td>4</td><td>5</td></tr>';
               $var=0;
   ...
   foreach ($invoices as $key) {
    ...
    // $html .=  '<option  value='. "'$key->invoice_no '". ">" . "$key->invoice_no" .'|'. "$key->inv_amount"  .   '</option> ';
   ...
   //$html .="</select>";
          ...
          $output .= '<tr><td><input class="chkidctl mt-4" type="checkbox"  name="invsel[chkid][]" value=0></td><td>' . $var . '</td><td>' . $key->invoice_no . '</td><td>' . $key->yr_month . '</td><td class="invamt">' . $key->inv_amount . '</td><td>' . $key->amt_paid_usd . '</td></tr>' ;
...
public function store(Request $request)
{
 ...
 $input = $request->all();
        ...
        if ($input['pay_option']<>'Direct'  && !isset($request->invoices))
        {

          return back()->withErrors(['errors' => ['Select Invoice no First']])->withInput();
        }

   
       $tot = DB::table('invoice_summary')->where('company_id','=', $request->company_id)->get(array(DB::raw('sum(inv_amount) as invamt'), DB::raw('sum(discount) as discount') ,DB::raw('sum(amt_paid_usd) as paidamt')));
 ...
 //dd($input);
   ...
   // $input_dtls   = $input['orddtls'];
    ...
    $company  =  DB::table('clients')->where('company_id', '=', $input['company_id'])->get();

    //dd($company);
    foreach ($company as $cm) {
      $client_company   =  $cm->client_company;
    }
   
    $input['client_company']=  $client_company;
    ...
    //dd($userid); die;
      ...
      //     }
        ...
        // }  
         ...
         $id = CompanyPaid::create($input)->id;

         //dd($id);

         $paid_amt = (float)$input['amt_paid_usd'];
        

        // $yearmonth =  date('Ym', $timestemp );
         $paid_yrmn = Carbon::parse($input['payment_date'])->format('Ym');
       //  $paid_yrmn =   date('Ym', $paid_yrmn );


        if ($input['pay_option']=='Direct') 
        {
            $get_prvpaid = DB::table('invoice_summary')->where('company_id','=', $company_id)->select('invoice_no', 'inv_amount', 'amt_paid_usd')->get();
             $bal_amt = 0;

             //  foreach ($get_prvpaid as $key) {
                
             // $prv_paid =  $key->amt_paid_usd;
             // $inv_amount = $key->inv_amount;
             // $invoice_no = $key->invoice_no;

             //  if($paid_amt > $inv_amount)
             //  {

             //        DB::table('invoice_summary')->where('company_id','=', $company_id)->where('invoice_no', '=', $invoice_no)->update(['amt_paid_usd'=>  $inv_amount ]);
             //  }
             //  else {
             //         DB::table('invoice_summary')->where('company_id','=', $company_id)->where('invoice_no', '=', $invoice_no)->update(['amt_paid_usd'=>  $paid_amt ]);
             //         break;

             //  }

             //        $paid_amt = $paid_amt- $inv_amount;
             //       }
         ...
         $get_prvpaid = DB::table('invoice_summary')->where('company_id','=', $company_id)->wherein('invoice_no', $invoices)->select('invoice_no', 'inv_amount', 'discount', 'amt_paid_usd')->get();
             $bal_amt = 0;
             ...
             if($paid_amt >= $inv_amount - $discount)
              {

                  DB::table('invoice_summary')->where('company_id','=', $company_id)->where('invoice_no', '=', $invoice_no)->update(['net_amt'=>$netamt, 'amt_paid_usd'=>  $paid_amt ]);
       ...
       $breaks = array("<br />","<br>","<br/>");  
        ...
        return redirect()->action('PaymentController@index', ['newid'=>$id]);
       ...
       // dd(Carbon::now());
         ...
         //dd($nowdt);
              ...
              //<th>Allocation </th>
               ...
               $url =  "<a href='orders/{$order->id}/edit'  > $order->order_id</a>" ;     
