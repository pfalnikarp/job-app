...
<div class="modal fade bd-example-modal-lg" data-backdrop="static" data-keyboard="false" tabindex="-1">
    ...
    <div class="col-md-3">
     ...
     @permission('file.price.dashboard')
            ...
            <tr><td>Vector<td>{{$weektotvectordata[$i][0]->totvect}} </td><td> {{$weektotvectordata[$i][0]->totvectprice}}</td></tr>
            <tr><td>Digitize<td>{{$weektotdigitdata[$i][0]->totdigit}} </td><td> {{$weektotdigitdata[$i][0]->totdigitprice}}</td></tr>
            <tr><td>Image Editing<td>{{$weekphotodata[$i][0]->dtotphoto}} </td><td> {{$weekphotodata[$i][0]->dtotphotoprice}}</td></tr>
            <tr><td>Alloted<td>{{$weektotalloted[$i][0]->totallot}} </td><td> {{$weektotalloted[$i][0]->totallotedprice}}</td></tr>
            <tr><td>QC-Pending  <td>{{$weektotqcpending[$i][0]->totqcpend}} </td><td> {{$weektotqcpending[$i][0]->totqcpendprice}}</td></tr>
            <tr><td>QC-OK<td>{{$weektotqc[$i][0]->totqcok}} </td><td> {{$weektotqc[$i][0]->totqcokprice}}</td></tr>
            <tr><td>Completed<td>{{$weektotcompl[$i][0]->totcompl}} </td><td> {{$weektotcompl[$i][0]->totcomplprice}}</td></tr>
            <tr><td>Revision<td>{{$weektotalrevesion[$i]}} </td><td> </td></tr>
            <tr><td>Changes<td>{{$weektotalchange[$i]}} </td><td></td></tr>
    @else
      <tr><td></td><td>File</td></tr>
            <tr><td>Vector</td><td>{{$weektotvectordata[$i][0]->totvect}} </td></tr>
            <tr><td>Digitize</td><td>{{$weektotdigitdata[$i][0]->totdigit}} </td></tr>
            <tr><td>Image Editing<td>{{$weekphotodata[$i][0]->dtotphoto}} </td></tr>
            <tr><td>Alloted</td><td>{{$weektotalloted[$i][0]->totallot}} </td></tr>
        <tr><td>QC-Pending</td><td>{{$weektotqcpending[$i][0]->totqcpend}} </td></tr>
            <tr><td>QC-OK</td><td>{{$weektotqc[$i][0]->totqcok}} </td></tr>
            <tr><td>Completed</td><td>{{$weektotcompl[$i][0]->totcompl}} </td></tr>
            <tr><td>Revision</td><td>{{$weektotalrevesion[$i]}} </td></tr>
            <tr><td>Changes</td><td>{{$weektotalchange[$i]}} </td></tr>
    @endpermission     
           </table>       
        </div>   
    </div>
