...
#invoices-sum-table_wrapper .dataTables_paginate .paginate_button {
...
table#invoices-sum-table tbody tr.selected {
    background-color: #e57373 !important;
    }

table#invoices-sum-table td a.glyphicon-edit:hover {
     color: orange !important;
}
...
/*table#invoices-sum-table thead {
    background: #00bfa5  !important;
}*/
...
table#invoices-sum-table tbody  tr {
...
table#invoices-sum-table td:hover{
    cursor: pointer;
...
/*table#invoices-sum-table   tr.redClass {
...
table#invoices-sum-table tbody td.osize {
...
/*table#invoices-sum-table td:hover::after,
table#invoices-sum-table th:hover::after {
  content: "";
  position: absolute;
...
select[name='invoices-sum-table_length'] {
  color: blue !important;
}
...
<div class="row">
    ...
    <div class ="col-md-5"> 
           <h4>CompanyWise Payment Summary</h4>
...
// $('#invoices-sum-table .fhead .firstrow th').each( function (i) {
//         var title = $('#invoices-sum-table th').eq( $(this).index() ).text();
...
//  var selected = [];    commented on 27/01/17  as  not know for what purpose kept
    ...
    $('#invoices-sum-table').DataTable({
        ...
        ajax: '{!! route('invoice-summary.summarygetindex') !!}',
      ...
      columnDefs: [
        ...
        columns: [
          { data: 'action', name: 'action', width: '5px', class: 'payedit', orderable: false, searchable: false },


           

          { data: 'client_company', name: 'client_company',  class: 'dt-body-left', 
                    // "render": function (e, type, full, meta) {
                    //         return '<span class="test" data-toggle="tooltip" title="' + data + '">' + data + '</span>';
                    //         }
            },
              { data: 'invoice_no', name: 'invoice_no' ,class: 'dt-center '
             },
          
             { data: 'yr_month', name: 'yr_month', class: 'dt-center '
             },
            { data: 'inv_amount', name: 'inv_amount', class: 'dt-body-right ', width: '50px'
             },
            

            { data: 'amt_paid_usd', name: 'amt_paid_usd' ,  class: 'dt-body-right ',width: '40px'
                       

             },
          
            { data: 'net_amt', name: 'net_amt' , class: 'dt-body-right ',width: '40px'

            },
                { data: 'pay_channel', name: 'pay_channel' ,  class: 'dt-body-right ',width: '40px'

             },
             { data: 'bank_charges', name: 'bank_charges' , class: 'dt-body-right ',width: '40px'

            },

              { data: 'conv_rate', name: 'conv_rate' , class: 'dt-body-right ',width: '40px'

            },
              { data: 'amt_received_inr', name: 'amt_received_inr' , class: 'dt-body-right ',width: '40px'

            },
            { data: 'paid_dt', name: 'paid_dt' , class: 'paiddt1 dt-body-right ', width: '40px'

             },
              { data: 'tran_id', name: 'tran_id' , class: ' dt-body-right ', width: '40px'

             },
             { data: 'remarks', name: 'remarks' , class: 'rmk', width: '40px'

             },
               { data: 'file1', name: 'file1' ,  width: '40px'

             },

               { data: 'file2', name: 'file2' ,  width: '10px'
 ...
 var table = $('#invoices-sum-table').DataTable();
   ...
   $('thead input[name="select_all"]', table.table().container()).on('click', function(e){
      if(this.checked){
         ...
         $('#invoices-sum-table tbody input[type="checkbox"]:checked').trigger('click');
      }
...
// new code for select all checkbox
   ...
   $('#invoices-sum-table tbody').on('click', 'input[type="checkbox"]', function(e){
      ...
      var tot = $($row).find('td.changeinv').text();
      alert(tot);
 ...
 if (rows_selected_total.length)
{
  ...
  //alert(sum1);
   ...
   $("#invoices-sum-table .fhead").on("click", "th.select-checkbox", function() {
           //  alert('hello');
           var cells = [];
        var rows = $("#invoices-sum-table").dataTable().fnGetNodes();
 ...
 $('#invoices-sum-table tbody').on( 'dblclick', 'td.payedit', function () {
        var href = $(this).find('a.glyphicon-edit').attr('href');
...
function normalrow() {
    var oSettings = jQuery('#invoices-sum-table').dataTable().fnSettings();
       ...
       //alert(updatedRow.html());
        ...
        $.ajaxSetup({
         ...
         if (columnIndex === 5) {
             ...
             alert('column 5 paid date changed');
...
$(document).ready(function(){
         ...
         $.ajax({
                            ...
                            { 
                             // res = result[0].msg ;
                              ...
                              // alert(result[0].client_company);

                              $("#editmodalinvsummary .myid").val(result[0].id);

                              $("#editmodalinvsummary .clientcomp").val(result[0].client_company);
                              ...
                              $("#editmodalinvsummary .bnkchrg").val(result[0].bank_charges);
                               ...
                               $("#editmodalinvsummary .oamt").val(result[0].out_amt);
  ...
  $("#editmodalinvsummary .pamt").change(function(){
       ...
       $inv_val = parseFloat($inv_val);

       //alert($inv_val);

       if($paid_val > $inv_val) {
           alert('Paid Amount should be less than Invoice Amount');
           return false;
       }
...
$(document).on("click", ".editinvsummary", function(e){
    e.preventDefault();
