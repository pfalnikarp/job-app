...
table#orders-table tbody tr.selected {
    background-color: #e57373 !important;
    }

table#orders-table td a.glyphicon-edit:hover {
     color: orange !important;
}
...
/*table#orders-table tbody tr td.FirstCol {
        background: transparent !important ;
}  
*/
...
table#orders-table tbody  tr.odd {
...
/*table#orders-table   tr.redClass {
...
table#orders-table tbody td.osize {
...
/*table#orders-table td:hover::after,
table#orders-table th:hover::after {
  content: "";
  position: absolute;
...
<table class="table table-responsive table-striped table-hover condensed searchHighlight maint" id="orders-table">
    <thead class = "fhead">
        <tr class="firstrow">
            <th >Select</th>
            <th>Order Id</th>
         ...
         <tr class="secondrow">
            <th style="width: 16px !important;">Select</th>
            <th>Order Id</th>
            <th>Order Date</th>
...
// $('#orders-table .fhead .firstrow th').each( function (i) {
//         var title = $('#orders-table th').eq( $(this).index() ).text();
        ...
        ajax: '{!! route('invoices.data') !!}',
         stateSaveCallback: function(settings,data) {
            ...
            createdRow: function ( row, data, index ) {
                  ...
                  if (( targetdt.getTime() < todaydt.getTime() )  && (data.status !== 'Approved') && (data.status !== 'Quote')   &&  (data.status !== 'Completed') && (data.status !== 'Cancel')  && (data.status !== 'Duplicate Entry'))  {
                            $(row).addClass( 'blink_me1' );
                           // $(row).css('background-color', '#F53535');
                          //  $(row).animate({ backgroundColor: "#F53535" }, {duration:300});
                   }

                   if (( targetdt.getTime() - todaydt.getTime()  < 30*60*1000 ) &&  (data.status !== 'Completed') && (data.status !== 'Approved') 
                    && (data.status !== 'Quote') &&  (data.status !== 'Cancel')  && (data.status !== 'Duplicate Entry'))  {

                          // alert( todaydt.getTime() - targetdt.getTime());
                            $(row).addClass( 'blink_me' );
                           // $(row).css('background-color', '#3B3605');
                            
                   }

                    if (( targetdt.getTime() - todaydt.getTime() > 30*60*1000 && targetdt.getTime() - todaydt.getTime() <= 120*60*1000) &&  (data.status !== 'Completed')  && (data.status !== 'Approved') && (data.status !== 'Quote') &&  (data.status !== 'Cancel')  && (data.status !== 'Duplicate Entry'))  {
                          
                            $(row).css('background-color', '#52E51F');
                   }

           
          },
        columns: [
          { data: 'action', name: 'action',  class: 'fstat ', orderable: false, searchable: false },
          { data: 'order_id', name: 'order_id', width: '10px'
            ...
            { data: 'target_completion_time', name: 'target_completion_time' 
                       , width: '130px', class: 'ord-comp-dt-tm',                                                                                                          
                    @if (isset($var))
                    @if (in_array('order-completion-date.hide', $var))
                           width: '0px', 
            ...
            @if (isset($var))
            @if (in_array('allowchange.allocation', $var))
                class: 'editalloc'
            ...
            @endif
                        ...
                        //  return ' {!! Form::select('allocation1[]', $users) !!}';
                        //     }

             },
            { data: 'stiches_count', name: 'stiches_count' },
            { data: 'file_price', name: 'file_price',
                  @if (isset($var))
                    @if (in_array('file-price.hide', $var)  || Auth::user()->hasRole('Designer'))
                          width: '0px', 
...
setInterval(blinker, 600);
  ...
  function rowgroup(){ 
        ...
        $("#orders-table").dataTable().rowGrouping({iGroupingColumnIndex:1,sGroupingColumnSortDirection: 'desc'});
   // Order by the grouping
    //$('#orders-table tbody').on( 'click', 'td.group', function (e) {
        //e.preventDefault();
        //alert('right click');
...
// });
 ...
 //code added  above on reality on 20-06-17
  ...
  $('#orders-table tbody').on( 'click', 'tr', function () {
   ...
   $('#orders-table tbody').on( 'dblclick', 'tr', function () {
        //alert('hello selected');
        var $rowid = table.row( this ).index() ;
        var currentPageIndex = table.page.info().page;
       //alert ($rowid);
...
function normalrow() {
    var oSettings = jQuery('#orders-table').dataTable().fnSettings();
...
$('#orders-table tbody').on('dblclick', 'td', function () {
  ...
  if ($('#orders-table thead tr.secondrow th').eq($(this).index()).text().trim() == 'File Note') {
   ...
   //   alert($('#orders-table thead tr.secondrow th').eq($(this).index()).text().trim()); changed on 21/06/17 
            // as logic changed

if ($('#orders-table thead tr.secondrow th').eq($(this).index()).text().trim() == 'Allocation') {
...
if ($('#orders-table thead tr.secondrow th').eq($(this).index()).text().trim() == 'File Count') {             
                        ...
                        $.ajax({
                            ...
                            console.log(result[0].msg) ; 
                             //$( "#filecountdialog .success" ).html("Data Updated Successfully");  
                              ...
                              //table.ajax.reload();
                               //$('#orders-table').DataTable().ajax.reload(); tried on 21/08/17 for reload 
                              table.page(currentPageIndex).draw(false);
...
if ($('#orders-table thead tr.secondrow th').eq($(this).index()).text().trim() == 'Order Date & Time') {             
...
if ($('#orders-table thead tr.secondrow th').eq($(this).index()).text().trim() == 'Order Comp.Date')  {   
   // alert('hi ok');
    var target = $(this);
...
if ($('#orders-table thead tr.secondrow th').eq($(this).index()).text().trim() == 'V.Emb.Rate') {
...
if ($('#orders-table thead tr.secondrow th').eq($(this).index()).text().trim() == 'V.File Price') {
  ...
  $('#dtltable #selectAll').click(function (e) {
   ...
   //$(document).on("change", "#dtltable select.childstatus", function(e){  
    ...
    $(document).on("focusin", "#dtltable select.childstatus", function(e){  
        ...
        previous = this.value;
       // alert(previous);
    }).on("change", "#dtltable select.childstatus", function() {    
        // Do something with the previous value after the change
...
$(document).on("click", ".editstatus", function(e){
   ...
   //debugger;
    ...
    // var url = '{!! route('ordertables.edit') !!}';
       ...
       $.ajax({
                ...
                data: {
                 ...
                 success: function (data) {
                     ...
                     $(".delmsg").html(data[0].msg);
                     $(".delmsg").html(data[0].msg).fadeOut( 60000 );
                     table.ajax.reload();
                 },
...
$(document).on("click", ".editaction", function(e){
  //e.preventDefault();
   alert('hi edit order details');
   var table = $('#orders-table').DataTable();
  var $rowid = table.row( this ).index() ;
  var currentPageIndex = table.page.info().page; 
  //alert($rowid);

  @if (!in_array('order-update', $var))
  ...
  @endif 
    ...
    // });
       ...
       var vloc =  window.location.pathname ;
    
        if (window.location.pathname == vloc) {
            // jQuery.noConflict();
            //alert(client_id);
            //var url = '/laravel/public/orders/'+client_id+ '/edit';
            //var url = '/laravel/public/ordertables/'+client_id ;
            //var url = 'ordertables';

            var url = '{!! route('orders.edit1') !!}';
            ...
            $.ajax({
                ...
                // },
                success: process_response_projects1,
                error: function (xhr) {
                    alert("AJAX request failed: " + xhr.status);
                }
            });
        ...
        else if (window.location.pathname == ('/ordertables/' + id)) {
            console.log(url);
            // jQuery.noConflict();
            var url = '{!! route('orders.edit1') !!}';
...
//  New FUNCTIONALITY IMPLEMENTED FOR DELAYED ORDERS ON 21/06/18 ABOVE

// new order editdtls modal on order id for multiple allocation
       ...
       var vloc =  window.location.pathname ;
        ...
        if (window.location.pathname == vloc) {
            ...
            $('#editmodalwindow').on('shown.bs.modal', function () {
              //alert("Modal Open");
             $("#editmodalwindow .clientinput").focus();
            });
